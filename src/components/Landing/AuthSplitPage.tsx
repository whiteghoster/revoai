...
export function AuthSplitPage({ mode }: AuthSplitPageProps) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f6f6ef] text-[#1E1B18]">
      <div className="grid min-h-screen lg:grid-cols-2">
        <aside className="relative hidden min-h-screen overflow-hidden lg:block">
          ...
          <Link href="/" className="absolute left-8 top-8 z-10 inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/15 px-4 py-2 text-sm font-bold text-white backdrop-blur-md hover:bg-white/20">
        ...
        <section className="relative flex min-h-screen items-center justify-center px-4 py-10 sm:px-6 lg:px-12">
          ...
          <div className="relative w-full max-w-md">
            <Link href="/" className="mb-8 inline-flex items-center gap-3 lg:hidden">
              <img src={branding.logo_url_light || branding.logo_url || "/logo.png"} alt={branding.app_name || "RevoAI"} className="h-10 w-auto" />
            </Link>
            ...
            <motion.div
              ...
              className="rounded-[32px] border border-[#E6E2D8] bg-white/90 p-6 shadow-[0_28px_90px_rgba(30,27,24,0.10)] backdrop-blur-xl sm:p-8"
            >
              <div className="mb-7 flex items-center justify-between gap-4">
                <div>
                  <span className="inline-flex rounded-full border border-[#FFD4B3] bg-[#FFF0E6] px-3 py-1 text-[11px] font-black uppercase tracking-wider text-[#FF7300]">
                    {isLogin ? "Login" : "Signup"}
                  </span>
                  <h2 className="mt-4 text-3xl font-black tracking-tight text-[#1E1B18]">{title}</h2>
                  <p className="mt-2 text-sm font-medium leading-relaxed text-[#655E56]">{subtitle}</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <>
                    <label className="block">
                      ...
                      <div className="relative">
                        <User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8C857D]" />
                        <input className="auth-input pl-11" required placeholder="John Smith" value={form.name} onChange={(e) => update("name", e.target.value)} />
                      </div>
                    </label>
                    <label className="block">
                      <span className="mb-1.5 block text-xs font-black text-[#1E1B18]">Company</span>
                      <div className="relative">
                        <Building2 className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8C857D]" />
                        <input className="auth-input pl-11" placeholder="Your company" value={form.company} onChange={(e) => update("company", e.target.value)} />
                      </div>
                    </label>
                  </>
                )}
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    <Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8C857D]" />
                    <input className="auth-input pl-11" type="email" required placeholder="you@company.com" value={form.email} onChange={(e) => update("email", e.target.value)} />
                  </div>
                </label>
                ...
                <label className="block">
                  ...
                  <div className="relative">
                    <LockKeyhole className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8C857D]" />
                    <input className="auth-input pl-11 pr-11" type={showPassword ? "text" : "password"} required placeholder="••••••••" value={form.password} onChange={(e) => update("password", e.target.value)} />
                    <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#8C857D] hover:text-[#FF7300]" aria-label="Toggle password visibility">
                ...
                <div className="flex items-center justify-between gap-3 text-xs font-bold text-[#655E56]">
                  {isLogin ? (
                    <label className="inline-flex items-center gap-2">
                      <input type="checkbox" className="h-4 w-4 accent-[#FF7300]" />
                      <span>Remember me</span>
                    </label>
                  ) : (
                    <label className="inline-flex items-center gap-2">
                      <input type="checkbox" className="h-4 w-4 accent-[#FF7300]" checked={form.acceptedTerms} onChange={(e) => update("acceptedTerms", e.target.checked)} required />
                ...
                <motion.button
                  ...
                  className="group mt-2 flex w-full items-center justify-center gap-2 rounded-full bg-[#FF7300] px-5 py-3.5 text-sm font-black text-white shadow-[0_16px_35px_rgba(255,115,0,0.22)] hover:bg-[#E66500] disabled:opacity-70"
                >

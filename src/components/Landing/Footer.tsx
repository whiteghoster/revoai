...
export function Footer() {
  ...
  return (
    <footer className="w-full bg-[#f6f6ef] text-[#4a4a4a] font-sans antialiased" data-testid="footer">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 items-start">
          ...
          <div className="space-y-5">
            <h2 className="text-2xl font-bold text-[#2d2d2d] tracking-tight">
              {branding.app_name || "RevoAI"}
            </h2>
            <p className="text-sm text-[#666666] leading-relaxed max-w-xs font-normal">
              {branding.app_tagline || "AI voice agents that qualify leads, book appointments, automate support, and sync with your business tools."}
            </p>
            {/* Round Minimal Social Indicators */}
            <div className="flex gap-2.5 pt-2">
              <a href="#" className="h-8 w-8 rounded-full bg-white hover:bg-[#FFF0E6] flex items-center justify-center text-[#555] hover:text-[#FF7300] transition-colors icon-hover border border-[#E6E2D8]">
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <ul className="space-y-3 text-sm text-[#555555]">
                <li>
                  <a href={`mailto:${branding.admin_email || "hello@revoai.co"}`} className="flex items-center gap-2 hover:text-black transition-colors font-medium">
                    <Mail className="h-4 w-4 stroke-[1.75]" />
                    {branding.admin_email || "hello@revoai.co"}
                  </a>
                </li>
                <li className="flex items-center gap-2 font-medium">
                  <MapPin className="h-4 w-4 stroke-[1.75]" />
                  <span>{branding.app_location || "Bengaluru, India"}</span>
                </li>
              </ul>
            </div>
            ...
            <form onSubmit={handleNewsletterSubmit} className="space-y-2.5 pt-2">
              ...
              <div className="flex max-w-xs relative items-center">
                <input
                  ...
                  className="w-full h-10 pl-4 pr-12 text-sm bg-white border border-transparent rounded-lg focus:outline-none focus:bg-white placeholder-[#8a827b] text-black"
                  required
                />
                <button
                  type="submit"
                  className="absolute right-1 top-1 bottom-1 px-3.5 bg-[#FF7300] hover:bg-[#E66500] text-white rounded-md transition-colors flex items-center justify-center"
        ...
        <div className="py-8 border-t border-[#E6E2D8] flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-medium text-[#7a7a7a]">
          <p>© {new Date().getFullYear()} {branding.app_name || "RevoAI"}. All rights reserved.</p>
